...
public class SlashBladeConfig {
    ...
    static {
        ...
        BEWITCHED_HUNGER_EXHAUSTION = COMMON_BUILDER.comment("Determining the base exhaustion for slashblade's self-repair.")
                .defineInRange("bewitched_hunger_exhaustion", 0.05D, 0.0001D, Double.MAX_VALUE);

        PLAYER_DAMAGE_MULTIPLIER = COMMON_BUILDER.comment("Determining the default overall damage multiplier for player.")
                .defineInRange("player_damage_multiplier", 1.0D, 0.0D, 512.0D);

        REFINE_DAMAGE_MULTIPLIER = COMMON_BUILDER.comment("Determining the damage multiplier of refine.")
                .defineInRange("refine_damage_multiplier", 0.785D, 0.0D, 512.0D);
